...
**Access fresh off-chain data**, e.g., trading strategies conditional on centralized exchange prices or transactions that are conditional on other transactions  
...
Creating Confidential Compute Requests can be done with the [Golang](/developers/golang-sdk) or [Typescript](/developers/typescript-sdk) SDKs.
...
If computing with confidential data (and without the same gas constraints) such that we can coordinate within the block times of other networks - with access to relevant offchain data - and therefore make credible commitments about the games we play doesn't excite you enough, then consider this.

**SUAVE is an attempt to keep power meaningfully decentralized** so that we can realize the promises of "crypto" (both cryptography and cryptocurrency): 

- censorship resistance and 
- permissionless participation. 

Irrespective of political stance or reason for being interested in crypto, we are all affected by the indirect consequences of our transactions. That is, _we are all affected by MEV_. Which means that MEV [calls into being a public](https://en.wikipedia.org/wiki/The_Public_and_Its_Problems) which transcends political ideology. Working to mitigate MEV therefore represents a way in which we can search for what John Dewey once called **"The Great Community"**, precisely because it is a community not premised on any particular ideology, but rather on an open marketplace for practical solutions to a problem we all face, regardless of background, beliefs, or personal preferences.
