...
In order to deploy this example contract, you need to have [SUAVE running locally](/explanations/run-suave/), which can be done from your git submodule directory:
...
That said, you can use Forge to deploy contracts to SUAVE (though we recommend that you then use the [Golang SDK](/explanations/interact-with-suave/golang-sdk) to send transactions).
