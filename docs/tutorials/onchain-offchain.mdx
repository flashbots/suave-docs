...
# On-chain Off-chain Programming Model
...
Now, for a Suapp which defines something like an orderflow auction, we want our off-chain component to emit information about user trades for searchers to construct backruns, as well as information about successful backruns merged with user trades for builders to construct blocks.
...
Congratulations! You have deployed a contract, sent it various confidential requests, and begun learning how to use `SUAVE-STD` to handle the requirements of programming Suapps with both on-chain and off-chain components! 💃 
