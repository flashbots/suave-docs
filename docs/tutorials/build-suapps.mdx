...
Setup what you need to handle contracts with `forge`:

```bash
cd packages/forge/ && forge install
```

Setup what you need to create the frontend with `suave-viem`:

```bash
cd packages/next/ && bun link viem
```

Build the contract and deploy them locally (from the root directory of your project):

```bash
yarn contracts:build 
```

You need to have SUAVE running locally for this to work:
...
4. We specify a new type for the transaction, which is exported from `suave-viem`.
...
What happens when we do actually want to send data in our CCR? The typescript template demonstrates how to do this by signing trasactions on Goerli that you wish to be processed by kettles on SUAVE.

In this case, we need to:

1. Craft a transaction on the domain where we want to transact and sign it. 
