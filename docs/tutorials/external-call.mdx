...
If you have SUAVE running locally, stop it and restart it with another flag we'll need for this tutorial:
...
Create a new contract in your `src` directory called `Chat.sol`. We'll follow exactly the same procedure to store our ChatGPT API key confidentially. In our `offchain()` function, we'll implement the few lines required to talk to GPT from your contract. In this example, we'll also pass an argument to our offchain function in order to let anyone use their own prompt:
...
contract Chat is Suapp {
    ...
    function offchain(string calldata prompt) external returns (bytes memory) {
        ...
        messages[0] = ChatGPT.Message(ChatGPT.Role.User, prompt);

        emit Response(chatgpt.complete(messages));
...
```bash
suave-geth spell conf-request <your_new_contract_address> 'offchain()' '("How do I build novel applications on SUAVE?")'
```
...
Feel free to post any Suapp you build on our [forum](https://collective.flashbots.net/c/suave/27) and we'll be happy to help you review and iterate, as well as invite you to our Developer Chat so you can see what others are building and share tips and best practices with the sharpest engineers we know.
