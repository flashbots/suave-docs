...
By crafting a network in which the interactions between each participant in the chain above can be encoded in builder solidity contracts, SUAVE becomes **open and contestable marketplace for mechanisms**. This is because Flashbots need design the perfect auction structures or fair ordering schemes; smart contract designers are now empowered to, using familiar, open source tools. 
