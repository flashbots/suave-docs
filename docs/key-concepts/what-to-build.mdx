...
One way to generate value is to design an app that is **uniquely enabled by SUAVE**.

What kind of apps are uniquely enabled by SUAVE? SUAVE exhibits three key differences to existing blockchains:

1. faster blocktime
2. support for programmable privacy
3. ability to do credible off-chain computation

[Interoperability with web2 services may be the key](https://www.notion.so/flashbots/Concrete-Ideas-On-Novel-SUAPPs-b6857caf58b94eaebf1852317594a219) to novel SUAPP design:

1. Big TAM, 
2. No cold-start problem, and 
3. Uniquely enabled by the previous three properties of SUAVE

Essentially, we could build [anti-mechanisms](https://www.youtube.com/watch?v=5v_wBkrIJhs) for web2 service. This is a fun, productive use of DarkDAOs (instead of building collusion rings to buy votes, we build collusion rings to allow permissionless interaction with web2). 
...
Ticketmasters sell things like Taylor Swift concert tickets. Currently there is huge complaint about their UX in that it’s basically “click the frontend, wait 10mins, and then have it says you didn’t get the ticket.” And the executives are worried.

If we do an application for this, it would be similar to ad auctions, but latency requirement is not as brutal, can be relaxed to like a few minutes.
...
2. Write path ORAM in [builder solidity](/key-concepts/builder-solidity). It should be [easy](https://pathoram.jimdofree.com/files/)
...
SUAVE got that privacy, Telegramgram bots today just escrow user keys. This increases the leverage on SGX, since now it matters for integrity (SGX breach could lead to stealing user funds), but that can't stop us from writing it.
