...
Builder solidity is a superset of Solidity with additional precompiles to support decentralized MEV applications. Using these precompiles, builder solidity contracts can define how SUAVE nodes run computation, without that computation being done on chain. 

That is, SUAVE nodes can accept encrypted data from users, do stuff with it privately and only reveal the results, not the inputs. This ensures confidentiality **and** improves efficiency, enabling computations that are not possible on blockchains like Ethereum.
...
Builder solidity enables anyone to create contracts that specify exactly how they will order bundles (and transactions) sent to them. This may help create incentives for block builders to implement their algorithms in verifiable, public contracts (that nevertheless preserve the confidentiality of any orderflow data they process). The reasons for this are multiple:
...
2. There are efficiency gains for users from aggregating and clearing their preferences inside the same auction. More users means more oderflow means more likelihood of landing blocks, or settling auctions, successfully.
