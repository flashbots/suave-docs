...
In present-day Ethereum, there are many centralized services used to coordinate and mitigate the effects of MEV on Blockchains and Decentralized Applications. Most of the categories below are based on these existing services:
...
But SUAVE also enables new and unique decentralized application paradigms.
...
RFQs, or Requests for Quote, are off-chain services that take a user's ask, broadcast it to a network of market makers, and respond with the best price. A SUAPP could do this in two ways: one is by using an HTTP post precompile to send to a list of market makers, or it could accept a curve of prices a market maker is willing to fill at and automatically respond on their behalf. This changes the nature of the RFQ as market makers would not need to continuously update new pricing curves.
