...
In SUAVE, we achieve this with [Kettles](/technical/specs/rigil/kettle#confidential-computation) performing compute offchain, but according to programs (smart contracts) written onchain. In this way, offchain compute is not constrained by chain consensus.
