...
This function starts a new builder instance within a Kettle. The basic idea is that you can use a session id (the `string` returned by the `newBuilder()` precompile) to help with simulating transactions efficiently. Opening a session enables you to build block iteratively, rather than having to re-run all your simulations each time you receive a new transaction or bundle. 
