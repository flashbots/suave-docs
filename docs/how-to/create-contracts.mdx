...
> TODO: this guide will be overhauled once we have Forge, Foundry and Viem working so I can create a tutorial + a how to guide and split this up more coherently.
...
1. I take my signed ETH L1 Uniswap tx, and encrypt it using the public key of a specific SUAVE computor. 
    1. This example assumes you know which SUAVE computor to encrypt it for. On the current testnet, only Flashbots is running nodes.
	2. My L1 transaction becomes the "request record" in another transaction to the MEVShare.sol on SUAVE.
2. I send the transaction, with the L1 request record, to the SUAVE computor I encrypted it for.
3. The SUAVE computor sees this transaction, sees that it contains a request record, and calls the MEVM running in that node to process confidentially.
...
1. The MEVM enters into the function being called in the MEVShare.sol contract on SUAVE.
    ...
    3. Once the MEVM has the data, it will use any other combination of precompiles to extract relevant parts of it and simulate the results of any computation done over that data.
    4. Relevant data is stored in the Confidential Data Store, under a specific "keyspace". The Confidential Data Store, and this notion of "keyspace", enables us to have public mechanisms specified in verifiable contracts which nevertheless collect and compute over private data. For instance:
		1. When MEVShare.sol tells the MEVM to combine transactions from users looking for protection with transactions from searchers doing backruns, the MEVM can do so without storing any of this data in MEVShare.sol's onchain storage.
		2. When EthBlockBid.sol tells the MEVM to access matched bids, it can also do, searching in its specific Confidential Data Store, in the same "mevshare" keyspace.
...
3. They craft their backrun on ETH L1, and wrap that backrun transaction into another transaction to the same SUAVE computor. 
...
Let's pick up where we were. I've submitted my MEVShare.sol transaction as a `confidentialComputeRequest`, specifying the SUAVE computor I want to send it to. The searcher has seen the `Hint` event emitted on chain and submitted their backrun transaction as another `confidentialComputeRequest` to the same node. That node, using the logic in MEVShare.sol, has combined these transactions (or "bids"), and stored them in the `confidentialDataStore` under the `mevshare:v0:mergedBids` keyspace.
