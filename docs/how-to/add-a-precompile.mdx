---
title: Add a precompile
description: How to add your own precompile to suave-geth such that it is accessible in any builder solidity contract on SUAVE
keywords:
  - reference
  - suave
  - precompiles
...
We specify the inputs and outputs for each precompile in `yaml`. From [this specification](https://github.com/flashbots/suave-geth/blob/main/suave/gen/suave_spec.yaml), two bindings are auto-generated, one in Solidity (the client) and another one in Go (the server).
...
We'll write a custom SUAVE precompile to perform the "add" operation in order to illustrate how to add your own. This is a two step process:
...
You will now need to write the logic required for your precompile to work as expected based on this generated skeleton. You can do this in the specific `suaveRuntime` struct generated in the same `contracts_suave_runtime_adaptor.go` file. In our case, the logic is very simple: just a straightforward addition of the values passed in as inputs. Your implementation may be arbitrarily more complex based on what you want the precompile to achieve.
