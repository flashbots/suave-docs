---
title: Deploy an Example SUAPP
description: Deploy a precompiled SUAPP and interact with it
---

The suave-geth repo has an [example version of MEV-Share](https://github.com/flashbots/suave-geth/blob/main/suave/sol/standard_peekers/bids.sol#L65) along with a script that will:
...
If you followed the [previous sub-section](/how-to/run-suave), you will have SUAVE running locally, either in Docker or via the binaries themselves.
...
[This script](https://github.com/flashbots/suave-geth/blob/main/suave/devenv/cmd/main.go) is focused on sending transactions in a local dev environment, but you can repurpose it for Rigil by changing the `exNodeNetAddr` and `exNodeEthAddr`.
