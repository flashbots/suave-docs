...
const wallet = suaveRigil.newWallet(
  PRIVATE_KEY,
  http(SUAVE_RPC_URL), // only needed if not using public testnet
);
...
If your SUAVE wallet doesn't have any ether, create a transaction object to fund the wallet with the required amount. On the [suave-geth local devnet](/docs/how-to/run-suave), the account `0xBE69d72ca5f88aCba033a063dF5DBe43a4148De0` is funded with 10 billion ETH, which you can use for testing.
