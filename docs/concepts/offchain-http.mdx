...
Given that code specified in contracts on SUAVE are executed in TEEs, which can provide attestations about the code they are running, we can more securely make requests to arbitrary http endpoints to fetch data relevant to any offchain computation we are running. 
